...
//	---------------- 페이지 처리 end ----------------
	...
	ArrayList<BookProfileDTO> list = dao.getSelectAll(param);
	dao.close();
%>
	...
	<tr>
		<td align="center">
			<select name="searchField">
				<option value="profileName">분류명</option>
			</select>
...
<% } else { 
	...
	int countNum = 0;
	for(BookProfileDTO dto : list) {
		//num = totalCount--;
		num = totalCount - (((pageNum - 1) * pageSize) + countNum++);
%>
	<tr align="center">
		<td><%= num %></td>
		<td align="center"><%= dto.getProfileName() %></td>
		<td align="center"><%= dto.getRegiDate() %></td>
		<td align="center">
			<a href="Edit.jsp?profileNo=<%= dto.getProfileNo() %>">[수정]</a>&nbsp;
			<a href="Delete.jsp?profileNo=<%= dto.getProfileNo() %>">[삭제]</a>
		</td>
	</tr>
<%
	}
